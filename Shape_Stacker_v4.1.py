...
def setDesert(level, x0, z0):
    ...
    # Desert ID is - 2
    biome = 2 # Paste here needed biome ID, list: https://minecraft.fandom.com/wiki/Biome/IDs_before_1.13
    minx = x0 - 20
    maxx = x0 + 21
    minz = z0 - 20
    maxz = z0 + 21
            ...
            for z in xrange(z0, z0 - r, -bxDZ):
                if hypot(x - x0, z - z0) < r and \
                   ...
                   hypot(x - x0 + bxDX, z-z0 + bxDZ) < r:
                    copyFromBox(level, x + (Xoffset * i), y0, z, options)
                i += 1
            i = 0
            for z in xrange(z0, z0 + r, bxDZ):
                 if hypot(x - x0, z - z0) < r and \
